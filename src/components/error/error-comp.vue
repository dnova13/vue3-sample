...
<script>
export default {
    // 230519 YJ-SEOL 에러페이지에서 tap-top의 ci클릭 시 mainHome으로 이동방지
    beforeRouteLeave(to, from, next) {
        // 이전 페이지 이동만 허용
        if (from.name !== null) {
            next()
        }
        // 라우터 이동을 막기 위해 next(false)를 호출
        next(false)
    },
    data() {
        return {}
    ...
    methods: {},
    mounted() {
        this.$emit('setTrigger', '/error')

        // YJ SEOL 230502 app.js setTrigger에 추가
